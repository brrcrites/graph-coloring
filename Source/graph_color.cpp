...
vector<string> GraphColoring::GraphColor::split(string to_split) {
...
void GraphColoring::GraphColor::parse_edge_list(char* input_file) {
    ...
    if(file.is_open()) {
        ...
        while(!flag && Getline(file,line)) {
            ...
            vector<string> words = GraphColoring::GraphColor::split(line);
            if(words.size() != 0) {
                if(words[0] == "p") {
...
void GraphColoring::GraphColor::parse_edge_matrix(char* input_file) {
    string pre = "v";
...
vector< vector<string> > GraphColoring::GraphColor::get_input(char* input_file) {
